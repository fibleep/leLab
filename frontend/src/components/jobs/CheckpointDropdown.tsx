...
export const CheckpointDropdown: React.FC<Props> = ({
  ...
  return (
    ...
    >
      <SelectTrigger
        className="bg-subtle border-line text-ink font-mono h-8 text-xs px-2 w-auto min-w-[110px]"
        onClick={(e) => e.stopPropagation()}
      >
        <SelectValue placeholder={placeholder} />
      </SelectTrigger>
      <SelectContent className="bg-elevated border-line text-ink font-mono text-xs">
        {checkpoints.map((c) => (
          <SelectItem
            key={c.step}
            value={String(c.step)}
